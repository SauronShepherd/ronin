...
PR qualification exposed that the coverage JSON keeps the unrounded value for `sqlite.py`: **58.91891891891892%**. The ratchet uses that measured value rather than either the integer table display or a two-decimal truncation. This is important because a two-decimal baseline of 58.92% is slightly higher than the actual observed measurement and therefore fails even though both values format as 58.92%.
...
- explicit legacy baseline: **58.91891891891892%** for `sqlite.py` only;
...
This makes the canonical SQLite baseline a visible transitional debt item rather than weakening the existing aggregate T2 >=90% policy.
...
1. raise canonical `sqlite.py` coverage from its explicit 58.91891891891892% legacy baseline toward the normal >=80% file floor with strong behavior tests;
...
6. retire the `sqlite.py` exception as soon as the canonical adapter meets the normal floor.
