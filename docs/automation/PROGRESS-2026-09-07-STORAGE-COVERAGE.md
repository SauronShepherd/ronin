...
Exact-main CI #610 on `b40626ba9cf773441ec523cedfbb10e2b2c7493b` reported these storage line/branch combined coverage figures:
...
- explicit legacy baseline: **59%** for `sqlite.py` only;
...
This makes `sqlite.py=59` a visible transitional debt item rather than weakening the existing aggregate T2 >=90% policy.
...
1. raise canonical `sqlite.py` coverage from its explicit 59% legacy baseline toward the normal >=80% file floor with strong behavior tests;
...
6. retire the `sqlite.py=59` exception as soon as the canonical adapter meets the normal floor.
