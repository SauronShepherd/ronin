...
- every explicit Core Metadata `License-File` path exposed by the installed distribution;
- installed PEP 639 `.dist-info/licenses/` files and conventional license/copying/notice/authors/copyright files, plus SHA-256 of each file's exact installed bytes;
