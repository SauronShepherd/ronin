...
| Local/Compose/Kubernetes | partial | local/Compose foundation, PostgreSQL metadata healthcheck, PostgreSQL JobStore adapter wired to the server, SQLite backup/restore, S3-compatible artifacts and constrained single-replica Helm chart exist; PostgreSQL HA and real-backend/production qualification remain |
