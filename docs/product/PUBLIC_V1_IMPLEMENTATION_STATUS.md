...
| Workspace/project/source control | partial | workspace/project persistence, authenticated create/read/update/archive workspace and project lifecycles with idempotency, audit and active-project filtering, project manifest, reproducible secret-free deployment manifest with canonical identity, Git identity, environment/deployment bindings, Studio journey covering project creation, environment binding, Bundle export/archive, governed pipeline execution and project-scoped permission inspection, plus matching SDK support; full Public v1 APIs/UI and membership remain |
