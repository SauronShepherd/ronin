...
def test_publish_creates_asset_revision_and_lineage_idempotently(tmp_path: Path) -> None:
    ...
    result = generate(
        GenerationPlan((TableSpec("customers", (ColumnSpec("id", "integer"),), rows=2),), seed=4)
    )
...
def test_publish_cannot_cross_workspace(tmp_path: Path) -> None:
    ...
    result = generate(
        GenerationPlan((TableSpec("customers", (ColumnSpec("id", "integer"),), rows=1),))
    )
    publisher = SyntheticOutputPublisher(catalog)
    try:
        publisher.publish(
...
def test_composed_runtime_smoke_flow_generates_and_queries_governed_output(tmp_path: Path) -> None:
    ...
    plan = manager.compose(
        (PluginRecord(plugin.manifest, plugin, "smoke"),), services={"catalog_store": catalog}
    )
    host = PluginHost(manager, plan)
    payload = {
        "plan": {
            "tables": [
                {"name": "customers", "rows": 2, "columns": [{"name": "id", "kind": "integer"}]}
            ],
            "seed": 8,
        }
    }
    generated = host.invoke_route(
        "POST", "/v1/synthetic-data-studio/generate", body=payload, idempotency_key="smoke-1"
    )
    ...
    lineage = plugin.get_lineage(
        asset_id="synthetic.customers", body={"workspace_id": "workspace-a", "version": run_id}
    )
