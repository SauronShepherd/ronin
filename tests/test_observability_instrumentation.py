...
def test_instrumentation_records_success_and_failure() -> None:
    ...
    assert instrument_execution("job", lambda: 7, record_metric=metrics.append, record_event=events.append, observed_at=now) == 7
    assert metrics[0].name == "execution.job.duration_ms"
    with pytest.raises(RuntimeError):
        instrument_execution("job", lambda: (_ for _ in ()).throw(RuntimeError("boom")), record_metric=metrics.append, record_event=events.append, observed_at=now)
