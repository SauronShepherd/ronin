...
async function loadRuns() { const serial = ++state.requestSerial; const cursor = state.cursors.at(-1); try { notice("Loading runs…"); const params = new URLSearchParams({ limit: "50" }); const project = $("run-project").value.trim(); const stateFilter = $("run-state").value; if (project) params.set("project", project); if (stateFilter) params.set("state", stateFilter); $("run-filter-summary").textContent = `Scope: ${project || "all projects"} · ${stateFilter || "all states"}`; if (cursor) params.set("cursor", cursor); const page = await request(`/v1/jobs?${params}`); if (serial === state.requestSerial) { renderRuns(page); $("next").dataset.cursor = page.next_cursor || ""; $("next").disabled = !page.next_cursor; $("previous").disabled = state.cursors.length < 2; notice(""); } } catch (error) { if (serial === state.requestSerial) notice(error.status === 403 ? "Forbidden: your grant does not permit listing these projects." : error.message, true); } }
...
$("run-filters").onsubmit = (event) => { event.preventDefault(); state.cursors = [null]; loadRuns(); };
$("clear-filters").onclick = () => { $("run-project").value = ""; $("run-state").value = ""; state.cursors = [null]; loadRuns(); };
