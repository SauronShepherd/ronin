...
function renderGenAI() {
  ...
  document.querySelector('#genai-evaluate').addEventListener('click', async () => {
    ...
    const output = document.querySelector('#genai-result');
    try {
      const examples = JSON.parse(form.querySelector('[name="evaluation"]').value);
      output.textContent = json(await post(`/v1/workspaces/${workspace}/genai/rag/evaluations`, { examples }));
    } catch (error) { output.textContent = `RAG evaluation failed: ${esc(error.message)}`; }
  });
  ...
  document.querySelector('#genai-run').addEventListener('click', async () => {
    const form = document.querySelector('#genai-discovery-form');
    const workspace = encodeURIComponent(form.querySelector('[name="workspace"]').value);
    const agent = encodeURIComponent(form.querySelector('[name="agent"]').value);
    const output = document.querySelector('#genai-result');
