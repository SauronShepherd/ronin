...
document.addEventListener('click', async event => {
  ...
  try {
    ...
    else if (action === 'de-sql-execute' || action === 'de-sql-export') { const data = new FormData(document.querySelector('#de-sql-form')); const workspace = encodeURIComponent(data.get('workspace')); const project = encodeURIComponent(data.get('project')); const result = await post(`/v1/workspaces/${workspace}/projects/${project}/sql/query`, { sql: data.get('sql'), max_rows: 1000, profile: data.get('profile'), translation_policy: data.get('translation_policy') }); sqlHistory = [{ sql: String(data.get('sql')), profile: String(data.get('profile')) }, ...sqlHistory.filter(entry => entry.sql !== String(data.get('sql')) || entry.profile !== String(data.get('profile'))) ].slice(0, 10); localStorage.setItem(SQL_HISTORY_KEY, JSON.stringify(sqlHistory)); renderSqlHistory(); if (action === 'de-sql-export') { const columns = (result.columns || []).map(column => typeof column === 'string' ? column : column.name); const rows = result.rows || []; out.textContent = [columns.join(','), ...rows.map(row => row.map(value => JSON.stringify(value ?? '')).join('\n'))].join('\n'); } else out.textContent = json(result); }
