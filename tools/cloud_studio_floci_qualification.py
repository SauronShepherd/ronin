...
def fixture() -> CloudTopology:
    return CloudTopology(
        resources=(
            CloudResource("bucket", "aws.s3.bucket", "qualification-bucket", {"force_destroy": True}),
...
def qualify(endpoint: str, workspace: str, terraform: bool = False) -> dict[str, object]:
    ...
    if terraform:
        result.update(
            plan=backend.plan(topology), apply=backend.apply(topology),
            refresh=backend.refresh(), destroy=backend.destroy()
        )
    return result
...
def main() -> int:
    ...
    parser.add_argument("--terraform", action="store_true", help="also run the optional Terraform adapter")
