...
class EvidenceAvailability(StrEnum):
    """Durable/public availability state independent of physical storage location."""
...
class StoredEvidenceRef:
    ...
    Logical content identity is independent of ``storage_ref``. Missing and tombstoned
    records retain the last trustworthy identity. Unavailable records intentionally carry
    no fabricated digest, size, or locator and require a normalized reason.
    """
    ...
    def __post_init__(self) -> None:
        if not self.role or self.role != self.role.strip():
            raise ValueError("evidence role must be non-empty and trimmed")
        try:
            availability = EvidenceAvailability(self.availability)
        except ValueError as exc:
            raise ValueError("unsupported evidence availability") from exc
        ...
        if availability is EvidenceAvailability.UNAVAILABLE:
            ...
            if (
                ...
                raise ValueError("unavailable evidence requires a non-empty trimmed reason")
            return
    ...
    def public_payload(self) -> dict[str, object]:
        """Return the v1 public representation without a backend storage locator."""
