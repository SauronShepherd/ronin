...
def build_workflow_bundle_inventory(store: SchedulerDefinitionStore, workspace_id: WorkspaceId, workflow_ids: tuple[object, ...] | None = None) -> WorkflowBundleInventory:
    """Build a deterministic definition-only inventory for one workspace."""
    all_workflows = tuple(sorted(store.list_workflows(workspace_id), key=lambda item: item.id.value))
    ...
    for workflow in workflows:
        ...
        payloads.append(BundleFile(payload_path, WORKFLOW_MEDIA_TYPE, workflow.to_json().encode("utf-8")))
    for schedule in schedules:
        ...
        objects.append(BundleInventoryObject("schedule", logical_ref, payload_path, (workflow_ref,)))
        payloads.append(BundleFile(payload_path, SCHEDULE_MEDIA_TYPE, schedule.to_json().encode("utf-8")))
    inventory = BundleInventory(tuple(objects))
    inventory_file = BundleFile(BUNDLE_INVENTORY_PATH, INVENTORY_MEDIA_TYPE, encode_canonical_json(inventory.to_payload()))
    return WorkflowBundleInventory(inventory, tuple(sorted((inventory_file, *payloads), key=lambda item: item.path)))


def export_workflow_bundle(store: SchedulerDefinitionStore, workspace_id: WorkspaceId, workflow_ids: tuple[object, ...] | Path, path: Path | None = None) -> RoninBundleManifest:
    if path is None:
    ...
    return write_bundle(target, build_workflow_bundle_inventory(store, workspace_id, selection).files)
...
def plan_workflow_bundle_import(
    ...
    *,
    ...
    inventory_payload = read_bundle_payload(path, BUNDLE_INVENTORY_PATH, max_bytes=max_inventory_bytes, limits=limits)
    ...
    for item in inventory.objects:
        ...
        try:
            if item.kind == "workflow":
                ...
                existing = scheduler_store.get_workflow(workspace_id, workflow_value.id) if scheduler_store is not None and workspace_id is not None else None
                objects.append(StagedWorkflowObject("workflow", item.logical_ref, workflow_value, "noop" if existing == workflow_value else "collision" if existing is not None else "create", "workflow definition conflicts" if existing is not None and existing != workflow_value else None))
            elif item.kind == "schedule":
                if len(item.dependencies) != 1 or item.dependencies[0] not in known_refs or payload.file.media_type != SCHEDULE_MEDIA_TYPE:
                    raise ValueError("invalid schedule inventory entry")
                schedule_value = Schedule.from_json(payload.file.data.decode("utf-8"))
                existing_schedule = scheduler_store.get_schedule(workspace_id, schedule_value.id) if scheduler_store is not None and workspace_id is not None else None
                objects.append(StagedWorkflowObject("schedule", item.logical_ref, schedule_value, "noop" if existing_schedule == schedule_value else "collision" if existing_schedule is not None else "create", "schedule definition conflicts" if existing_schedule is not None and existing_schedule != schedule_value else None))
            else:
    ...
    workflows_sorted = tuple(cast(WorkflowDefinition, item.payload) for item in objects if item.kind == "workflow")
    schedules_sorted = tuple(cast(Schedule, item.payload) for item in objects if item.kind == "schedule")
    workflow_ids = {item.id for item in workflows_sorted}
    if len(workflow_ids) != len(workflows_sorted) or any(item.workflow_id not in workflow_ids for item in schedules_sorted):
...
__all__ = ("INVENTORY_MEDIA_TYPE", "SCHEDULE_MEDIA_TYPE", "WORKFLOW_MEDIA_TYPE", "WorkflowBundleImportPlan", "WorkflowBundleInventory", "build_workflow_bundle_inventory", "export_workflow_bundle", "plan_workflow_bundle_import")
