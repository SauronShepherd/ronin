...
def ingest_stream_record(store: SchedulerEventStore, record: StreamRecord, *, workspace_id: WorkspaceId, stream_id: str, received_at: Instant | str) -> tuple[EventDelivery, ...]:
    ...
    occurred = Instant("1970-01-01T00:00:00.000000Z") if record.timestamp_ms is None else Instant(datetime.fromtimestamp(record.timestamp_ms / 1000, UTC).strftime("%Y-%m-%dT%H:%M:%S.%fZ"))
    return store.ingest_event(SchedulerEventRecord(workspace_id, event_id, "stream.record", stream_id, record.key, digest, occurred, Instant(received_at)))
