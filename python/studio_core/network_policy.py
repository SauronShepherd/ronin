"""Pure shared policy for plaintext HTTP binding and client transport."""
...
def parse_bind_policy(value: str | None) -> BindPolicy:
    """Return a validated binding policy, defaulting to loopback-only."""
    if value is None:
        return "loopback"
...
def allows_plaintext_non_loopback(policy: BindPolicy) -> bool:
    """Return whether the declared deployment policy permits non-loopback plaintext."""
