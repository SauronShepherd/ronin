...
class ExperimentId:
    ...
    def __post_init__(self) -> None:
        _text(self.value, "experiment id")
...
class ModelVersion:
    ...
    def __post_init__(self) -> None:
        _text(self.value, "model version")
...
class Experiment:
    ...
    def __post_init__(self) -> None:
        ...
        if self.description is not None:
            _text(self.description, "experiment description")
...
class MLRunRecord:
    ...
    def __post_init__(self) -> None:
        ...
        if len(artifacts) != len(set(artifacts)):
            raise ValueError("ML artifact refs must be unique")
        if self.runtime_snapshot_ref is not None:
            _text(self.runtime_snapshot_ref, "runtime snapshot ref")
    ...
    def from_payload(cls, payload: object) -> MLRunRecord:
        ...
        if not isinstance(payload, Mapping) or set(payload) != expected:
            raise ValueError("ML run has invalid shape")
        if not all(
            isinstance(payload[k], str)
            for k in ("id", "experiment_id", "execution_ref", "source_revision")
        ):
            raise ValueError("ML run identity fields must be strings")
        datasets, parameters, metrics, artifacts, runtime = (
            payload["datasets"],
            payload["parameters"],
            payload["metrics"],
            payload["artifact_refs"],
            payload["runtime_snapshot_ref"],
        )
        if (
            ...
            raise ValueError("ML run collections have invalid types")
        if not isinstance(parameters, Mapping) or not all(
            isinstance(k, str) and isinstance(v, str) for k, v in parameters.items()
        ):
            raise ValueError("ML parameters must be string object")
        if not all(isinstance(v, str) for v in artifacts):
            raise ValueError("ML artifact refs must be strings")
        if runtime is not None and not isinstance(runtime, str):
            raise ValueError("runtime snapshot ref must be string or null")
        parsed_metrics: list[MetricValue] = []
        for item in metrics:
            if not isinstance(item, Mapping) or set(item) != {"name", "value", "step"}:
                raise ValueError("metric has invalid shape")
            ...
            if (
                ...
                raise ValueError("metric has invalid types")
            parsed_metrics.append(MetricValue(name, float(value), step))
        return cls(
            MLRunId(cast(str, payload["id"])),
            ExperimentId(cast(str, payload["experiment_id"])),
            cast(str, payload["execution_ref"]),
            cast(str, payload["source_revision"]),
            tuple(AssetRef.from_payload(item) for item in datasets),
            tuple(sorted(cast(Mapping[str, str], parameters).items())),
            tuple(parsed_metrics),
            tuple(cast(list[str], artifacts)),
            cast(str | None, runtime),
        )

    @classmethod
    def from_json(cls, payload: str) -> MLRunRecord:
        return cls.from_payload(decode_canonical_json(payload))
...
class RegisteredModelVersion:
    ...
    def __post_init__(self) -> None:
        _text(self.artifact_ref, "model artifact ref")
        _text(self.artifact_digest, "model artifact digest")
        _text(self.framework, "model framework")
        if self.stage not in {"candidate", "champion", "archived"}:
...
class ModelEvaluation:
    ...
    def __post_init__(self) -> None:
        ...
        metrics = tuple(sorted(self.metrics, key=lambda item: (item.name, item.step)))
        if not metrics:
            raise ValueError("model evaluation requires at least one metric")
        object.__setattr__(self, "metrics", metrics)
