...
class PromptVersion:
    ...
    def __post_init__(self) -> None:
        _text(self.value, "prompt version")
...
class VectorIndexId:
    ...
    def __post_init__(self) -> None:
        _text(self.value, "vector index id")
...
class ModelProvider:
    ...
    def __post_init__(self) -> None:
        ...
        if self.endpoint is not None:
            _text(self.endpoint, "provider endpoint")
    ...
    def to_payload(self) -> dict[str, object]:
        return {
            ...
            "secret_ref": None if self.secret_ref is None else self.secret_ref.uri,
...
class GenAIModel:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.capabilities or not self.capabilities.issubset(allowed):
            raise ValueError("unsupported/empty GenAI capabilities")
...
class PromptAsset:
    ...
    def __post_init__(self) -> None:
        if not self.template or "\x00" in self.template:
        ...
        if len(parameters) != len(set(parameters)):
            raise ValueError("prompt parameters must be unique")
        allowed = {"chat", "embedding", "tool_use", "structured_output"}
        if not self.required_capabilities.issubset(allowed):
            raise ValueError("unsupported prompt capability")
    ...
    def from_json(cls, payload: str) -> PromptAsset:
        ...
        if not isinstance(value, Mapping) or set(value) != {
            ...
            raise ValueError("prompt has invalid shape")
        if not all(isinstance(value[k], str) for k in ("id", "version", "template")):
            raise ValueError("prompt identity/template fields must be strings")
        params = value["parameter_names"]
        caps = value["required_capabilities"]
        if (
            not isinstance(params, list)
            or not all(isinstance(v, str) for v in params)
            or not isinstance(caps, list)
            or not all(isinstance(v, str) for v in caps)
        ):
            raise ValueError("prompt arrays invalid")
        return cls(
...
class VectorIndexDefinition:
    ...
    def __post_init__(self) -> None:
        ...
        metadata_fields = tuple(
            sorted(_text(v, "vector metadata field") for v in self.metadata_fields)
        )
        if not text_fields:
            raise ValueError("vector index requires text_fields")
        if len(text_fields) != len(set(text_fields)) or len(metadata_fields) != len(
            set(metadata_fields)
        ):
            raise ValueError("vector index fields must be unique")
        if self.chunk_size < 1 or self.chunk_overlap < 0 or self.chunk_overlap >= self.chunk_size:
            raise ValueError("invalid vector chunk sizing")
        object.__setattr__(self, "text_fields", text_fields)
...
class RAGDefinition:
    ...
    def __post_init__(self) -> None:
        _text(self.name, "RAG name")
        _text(self.model_id, "RAG model id")
        if self.top_k < 1 or self.top_k > 100:
            raise ValueError("RAG top_k must be between 1 and 100")

    def to_payload(self) -> dict[str, object]:
        return {
            "name": self.name,
            "index_id": self.index_id.value,
...
class ToolContract:
    ...
    def __post_init__(self) -> None:
        ...
        _text(self.input_schema_ref, "tool input schema ref")
        _text(self.output_schema_ref, "tool output schema ref")
