...
def translate_notebook_job(
        ...
        if isinstance(notebook, Mapping) and isinstance(notebook.get("notebook_path"), str):
            node = Node.create(
                ...
                params={
                    ...
                    "parameters": notebook.get("base_parameters", {}),
                },
                inputs=(Port("in"),) if dependency_names else (),
                outputs=(Port("out"),),
            )
            nodes.append(node)
            ...
            reports.append(
                MigrationObjectReport(
                    ...
                    task_key,
                    "translated",
                    (f"workflow-node:{task_key}",),
                    ("Databricks notebook task translated to Ronin notebook.run",),
                )
            )
        else:
            reports.append(
                MigrationObjectReport(
                    ...
                    "unsupported",
                    (),
                    ("Only notebook_task is executable in this translation subset",),
                )
            )
    if not nodes:
    ...
    workflow = WorkflowDefinition(
        WorkflowId(f"databricks-job-{job_id}"),
        str(payload.get("name", job_id)),
        Pipeline(tuple(nodes), tuple(edges)),
    )
    reports.append(
        MigrationObjectReport(
            "job",
    ...
    if schedule_value is not None:
        if not isinstance(schedule_value, Mapping) or not isinstance(
            schedule_value.get("cron"), str
        ):
            raise ValueError("Databricks schedule requires a cron string")
        schedule = Schedule(
            ScheduleId(f"databricks-schedule-{job_id}"), workflow.id, schedule_value["cron"]
        )
    return DatabricksTranslation(
        workflow,
        MigrationReport("databricks", source_version, "ronin-databricks-0.1", tuple(reports)),
        schedule,
    )
