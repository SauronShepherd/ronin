...
def translate_code_recipes(
    ...
    if (
        ...
        or not isinstance(payload.get("recipes"), Sequence)
        or isinstance(payload["recipes"], (str, bytes))
    ):
        ...
        if kind == "python" and isinstance(source, str):
            ...
            reports.append(
                MigrationObjectReport(
                    ...
                    "unsupported",
                    (),
                    ("Only explicit Python and SQL recipe sources are executable in this subset",),
                )
            )
            continue
        nodes.append(
            Node.create(
                operator=OperatorRef(operator), instance_key=recipe_id, params={"source": source}
            )
        )
        reports.append(
            MigrationObjectReport(
                ...
                recipe_id,
                "translated",
                (f"workflow-node:{recipe_id}",),
                (f"Dataiku {kind} recipe translated to {operator}",),
            )
        )
    if not nodes:
        raise ValueError("Dataiku export contains no executable code recipes")
    workflow = WorkflowDefinition(
        WorkflowId(f"dataiku-project-{project_id}"),
        str(payload.get("name", project_id)),
        Pipeline(tuple(nodes)),
    )
    return DataikuTranslation(
        workflow,
        MigrationReport("dataiku-dss", source_version, "ronin-dataiku-0.1", tuple(reports)),
    )
