...
class SourceObject:
    ...
    def report(self) -> MigrationObjectReport:
        return MigrationObjectReport(
            ...
            self.notes
            or (
                ("No executable Ronin target mapping is registered",)
...
class MigrationInventory:
    ...
    def add(self, obj: SourceObject) -> None:
        if any(
            existing.object_type == obj.object_type and existing.object_id == obj.object_id
...
def inventory_json_document(
    ...
    *,
    ...
    if (
        ...
        or not isinstance(payload.get("objects"), Sequence)
        or isinstance(payload["objects"], (str, bytes))
    ):
    ...
    for item in payload["objects"]:
        ...
        inventory.add(
            SourceObject(
                str(item["type"]), str(item["id"]), status, (), tuple(notes), tuple(bindings)
            )
        )
