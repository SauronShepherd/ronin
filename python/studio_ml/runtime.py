...
def _model_parameters(model: object, spec: TrainingSpec) -> dict[str, object]:
    if spec.algorithm == "linear_regression":
        ...
        coefficients = [_finite_float(value, "linear coefficient") for value in coefficients_raw.tolist()]
        return {
...
def _dot(coefficients: Sequence[float], vector: Sequence[float], intercept: float) -> float:
    return sum(coefficient * value for coefficient, value in zip(coefficients, vector, strict=True)) + intercept


def _linear_predictions(parameters: Mapping[str, object], matrix: Sequence[Sequence[float]], feature_count: int) -> tuple[float, ...]:
...
def _logistic_predictions(
    ...
    for vector in matrix:
        scores = tuple(
            _dot(coefficients[index], vector, intercepts[index])
